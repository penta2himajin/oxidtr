...
fn generate_constraints(ir: &OxidtrIR, _ctx: &LeanContext) -> String {
    ...
    let mut out = String::new();
    writeln!(out, "-- Generated by oxidtr (Lean 4 backend)").unwrap();
    writeln!(out, "-- Constraint theorems: replace `sorry` with proofs").unwrap();
    ...
    for c in &sig_constraints {
        match c {
            analyze::ConstraintInfo::NoSelfRef { sig_name, field_name } => {
                ...
                writeln!(out, "    ∀ (x : {sig_name}), x.{fname} ≠ some x :=").unwrap();
            ...
            analyze::ConstraintInfo::Acyclic { sig_name, field_name } => {
                ...
                writeln!(out, "    ∀ (x : {sig_name}), ¬ Relation.TransGen (fun a b => a.{fname} = some b) x x :=").unwrap();
                writeln!(out, "  sorry").unwrap();
                writeln!(out).unwrap();
                theorem_idx += 1;
            }
            analyze::ConstraintInfo::FieldOrdering { sig_name, left_field, op, right_field } => {
                ...
                writeln!(out, "    ∀ (x : {sig_name}), x.{lf} {lean_op} x.{rf} :=").unwrap();
                writeln!(out, "  sorry").unwrap();
                writeln!(out).unwrap();
                theorem_idx += 1;
            }
            analyze::ConstraintInfo::Implication { sig_name, condition, consequent } => {
                ...
                writeln!(out, "    ∀ (x : {sig_name}), {cond_str} → {cons_str} :=").unwrap();
                writeln!(out, "  sorry").unwrap();
                writeln!(out).unwrap();
                theorem_idx += 1;
            }
            analyze::ConstraintInfo::Iff { sig_name, left, right } => {
                ...
                writeln!(out, "    ∀ (x : {sig_name}), {left_str} ↔ {right_str} :=").unwrap();
                writeln!(out, "  sorry").unwrap();
                writeln!(out).unwrap();
                theorem_idx += 1;
            }
            analyze::ConstraintInfo::Prohibition { sig_name, condition } => {
                ...
                writeln!(out, "    ∀ (x : {sig_name}), ¬({cond_str}) :=").unwrap();
                writeln!(out, "  sorry").unwrap();
                writeln!(out).unwrap();
                theorem_idx += 1;
            }
            analyze::ConstraintInfo::Disjoint { sig_name, left, right } => {
                ...
                writeln!(out, "    ∀ (x : {sig_name}), ¬(x ∈ {left} ∧ x ∈ {right}) :=").unwrap();
                writeln!(out, "  sorry").unwrap();
                writeln!(out).unwrap();
                theorem_idx += 1;
            }
            analyze::ConstraintInfo::Exhaustive { sig_name, categories } => {
                ...
                writeln!(out, "    ∀ (x : {sig_name}), {cats} :=").unwrap();
                writeln!(out, "  sorry").unwrap();
                writeln!(out).unwrap();
                theorem_idx += 1;
            }
            analyze::ConstraintInfo::CardinalityBound { sig_name, field_name, bound } => {
                ...
                writeln!(out, "    ∀ (x : {sig_name}), {bound_str} :=").unwrap();
                writeln!(out, "  sorry").unwrap();
                writeln!(out).unwrap();
                theorem_idx += 1;
            }
