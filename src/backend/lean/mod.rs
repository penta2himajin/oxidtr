...
fn generate_operations(ir: &OxidtrIR) -> String {
    ...
    // Callee before caller. ponytail: a call cycle would need `mutual` plus a
    // termination proof; emit those in model order and let Lean report it.
    let mut pending: Vec<&OperationNode> = ir.operations.iter()
        .filter(|op| op.receiver_sig.is_none()).collect();
